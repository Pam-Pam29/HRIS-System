...
export class MockEmployeeService implements IEmployeeService {
  ...
  async createEmployee(employee: Omit<Employee, 'id'>): Promise<Employee> {
    const newEmployee = { ...employee, id: Date.now().toString() } as Employee;
...
// Default export - can be easily changed
export const employeeService = EmployeeServiceFactory.createService('mock');
